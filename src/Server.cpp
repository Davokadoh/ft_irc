...
Server::Server(const std::string &port, const std::string &password)
    : _password(password), _port(port), _name(":ft_irc.42.ch"), _status(true),
      _maxSd(1) {
  FD_ZERO(&_mainSet);
  FD_ZERO(&_recvSet);
  FD_ZERO(&_sendSet);
...
void Server::execute(Client &client) {
  ...
  client.setSource(":" + client.getNickname() + "!" + client.getUsername() +
                   "@" + client.getIp());
  if (verb.empty())
    return;
    ...
    client.sendMessage(this->_name +
                       ERR_UNKNOWNCOMMAND(client.getNickname(), verb));
}
...
void Server::registration(Client &client) {
  ...
  client.sendMessage(_name +
                     RPL_CREATED(client.getNickname(), getDateCreation()));
  ...
  client.sendMessage(_name +
                     RPL_LUSERCHANNELS(client.getNickname(), nbrChannels));
...
void Server::partChannels(Client &client) {
  for (std::map<std::string, Channel *>::iterator it = _channels.begin();
    ...
    if (it->second->isClient(&client)) {
      ...
      it->second->sendToAll(
          client.getSource() + " PART " + it->first + " :Leaving", NULL);
